...
def move_square_server():
	rospy.init_node('move_pentagon_server',anonymous = True)
	s = rospy.Service( 'move_pentagon', MovePentagon, handle_move_square )
	rospy.spin()
...
def rotate(vel_msg,pub):
	...
	while ( angle_travelled < 1.25663706 ):
		pub.publish(vel_msg)
		t1 = rospy.Time.now().to_sec()
